...
resource "aws_iam_policy" "read_access_dynamodb" {
  name = "${var.application-name}-data-read-access-${var.env}"
  path = "/acct-managed/"
...
data "aws_iam_policy_document" "data_read_access_doc" {
  policy_id = "${var.application-name}-data-read-access-doc-${var.env}"
  statement {
    sid = "1"
    actions = [
...
resource "aws_iam_policy" "data_readwrite_access" {
  name = "${var.application-name}-data-readwrite-access-${var.env}"
  path = "/acct-managed/"
...
data "aws_iam_policy_document" "readwrite_access_dynamodb_doc" {
  policy_id = "${var.application-name}-data-readwrite-access-doc-${var.env}"
  statement {
    sid = "1"
    actions = [
      "dynamodb:DescribeTimeToLive"
